  ...
  function getColorConclusion() {
    ...
    if (filteredCommitsObject != null) {
      const colors = filteredCommitsObject.map((commit) => {
        // Si hay cobertura, asumimos que el commit fue exitoso
        if (commit.coverage !== null && commit.coverage !== undefined) {
          return containsRefactor(commit.commit.message) ? "blue" : "green";
        } else {
          // Sin información de cobertura, asumimos que el commit falló o no se probó
          return "black";
        }
      });
